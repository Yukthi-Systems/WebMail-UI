...
import {
  ...
  FaSync,
  FaLock,
} from 'react-icons/fa'; // Added service icons
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-white rounded-2xl border border-gray-200 p-8 sm:p-10 shadow-sm">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-sm text-gray-600 text-center">Sign in to continue to Webmail</p>
          </div>
          ...
          <FormProvider {...methods}>
            <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-5">
              ...
              <button
                ...
                style={{
                  backgroundColor: isLoading ? undefined : PRIMARY_COLOR[600],
                }}
                onMouseEnter={(e) => {
                  if (!isLoading) {
        ...
        <div className="mt-6 px-4">
          ...
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-3 mb-4">
            {[
              ...
              { href: 'https://mail.mail25.info/', icon: MdMailOutline, label: 'Old WebMail - 1' },
              { href: 'https://mail25.info/', icon: MdMailOutline, label: 'Old WebMail - 2' },
            ].map((service, index) => (
              <a
                key={index}
                href={service.href}
          ...
          {/* Manage Domain Link */}
          <div className="mt-6 pt-2 border-t border-gray-100/50 flex justify-center">
            <Link
              to="/1219/admin/login"
              className="inline-flex items-center gap-2 text-xs text-gray-400 hover:text-orange-600 transition-colors duration-200 group"
            >
              <span className="font-medium">Configure Domain</span>
            </Link>
          </div>
        </div>
        ...
        <div className="mt-4 text-center">
          <p className="text-[10px] text-gray-400 font-medium tracking-wide">
            © {new Date().getFullYear()} MAIL25 SYSTEMS
          </p>
        </div>
      </div>
      <p className="absolute right-14 bottom-8 text-[11px] text-gray-400">v{API_CONFIG.version}</p>
    </div>
  );
};
